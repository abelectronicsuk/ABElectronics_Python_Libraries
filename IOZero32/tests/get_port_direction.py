...
"""
================================================
AB Electronics UK IO Zero 32 Tests | test get_port_direction function
...
This test validates the get_port_direction function in the IOZero32 class.
...
def main():
    ...
    for a in range(1, 256):
        io_bus.set_port_direction(0, a)
        x = io_bus.get_port_direction(0)
        if x != a:
            passed = False
            break
        io_bus.set_port_direction(1, a)
        x = io_bus.get_port_direction(1)
        if x != a:
            passed = False
            break
