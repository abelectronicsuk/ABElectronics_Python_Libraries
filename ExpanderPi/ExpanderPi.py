...
class IO:
    ...
    def set_pin_direction(self, pin, direction):
        ...
        pin = pin - 1
        if pin < 8:
            self.__port_a[0] = self.__helper.updatebyte(
                self.__port_a[0], pin, direction)
            self.__bus.write_byte_data(
                self.__ioaddress, self.IODIRA, self.__port_a[0])
        else:
            self.__port_b[0] = self.__helper.updatebyte(
    ...
    def set_pin_pullup(self, pin, value):
        ...
        pin = pin - 1
        if pin < 8:
            self.__port_a[2] = self.__helper.updatebyte(
                self.__port_a[2], pin, value)
            self.__bus.write_byte_data(
                self.__ioaddress, self.GPPUA, self.__port_a[2])
        else:
            self.__port_b[2] = self.__helper.updatebyte(
    ...
    def write_pin(self, pin, value):
        ...
        pin = pin - 1
        if pin < 8:
            self.__port_a[1] = self.__helper.updatebyte(
                self.__port_a[1], pin, value)
            self.__bus.write_byte_data(
                self.__ioaddress, self.GPIOA, self.__port_a[1])
        else:
            self.__port_b[1] = self.__helper.updatebyte(
    ...
    def invert_pin(self, pin, polarity):
        ...
        pin = pin - 1
        if pin < 8:
            self.__port_a[3] = self.__helper.updatebyte(
                self.__port_a[1],
                pin,
                polarity)
            self.__bus.write_byte_data(
            ...
            self.__port_b[3] = self.__helper.updatebyte(
                self.__port_b[1],
                pin - 8,
                polarity)
            self.__bus.write_byte_data(
